...
"@
        ...
        if ((Test-Connection -TargetName $Hostname -Count 4 -Ping).Status -eq "Success") {
...
"@
        ...
        if ((Test-Connection -TargetName $Hostname -TcpPort $Port) -eq "True" ) {
            $StatusUpdate = $StatusUpdate.Replace("[TABLE]","$Table")
            $StatusUpdate = $StatusUpdate.Replace("[STATUS]","Available")
            $StatusUpdate = $StatusUpdate.Replace("[ID]","$Id")
            $StatusUpdate = $StatusUpdate.Replace("[LASTUPDATE]","$Date")
...
"@
    $Seperator = "::"
    $Resolve_message = "Resolved: $Hostname$Seperator$Port [$Date UTC]"
